...
N 720 -760 790 -760 {lab=s}
N 1050 -760 1080 -760 {lab=g_csc}
N 760 -460 760 -400 {lab=vm2}
N 760 -340 760 -280 {lab=rf_in_pad}
...
C {capa.sym} 920 -760 3 0 {name=CNC
m=1
value=\{CNC_VAL\}
footprint=1206
device="ceramic capacitor"
}
C {lab_pin.sym} 1080 -760 2 0 {name=p68 sig_type=std_logic lab=g_csc}
C {lab_pin.sym} 720 -760 0 0 {name=p69 sig_type=std_logic lab=s}
...
"}
C {code_shown.sym} 1670 80 0 0 {name=s1 only_toplevel=false value="
...
C {code.sym} 362.5 413.75 0 0 {name=s5 only_toplevel=false value="
...
device=inductor
spice_ignore=short}
C {code.sym} 312.5 783.75 0 0 {name=s2 only_toplevel=false value="
.control

* =============================================================================
* GF180MCU LNA: PAD + BOOTSTRAP + ACTIVE NOISE-CANCELLATION COARSE SWEEP
*
* REQUIRED SCHEMATIC ELEMENTS:
*   CBOOT vm2 rf_in_pad \{CBOOT_VAL\}
*   CNC   s   net11     \{CNC_VAL\}
*   R_FF  g_csc net11   \{R_FF\}
*
* The sweep keeps:
*   L_IN      = 1 nH
*   R_L_IN    = 1.257 ohm  (Q approximately 12 at 2.4 GHz)
*   L_BW      = 2 nH
*   R_BW      = 0.2 ohm
*   VBIAS_CG  = 2.25 V
*
* It sweeps:
*   M_CG       = 7, 8, 9, 10, 11
*   CBOOT_VAL  = 75, 150, 225, 300, 375 fF
*   CIN_VAL    = 4.7, 10, 15, 22 pF
*   CNC_VAL    = 1.0, 2.2, 3.3, 4.7 pF
*   R_FF       = 22, 47, 68 ohm
*
* Total ON combinations: 5 x 5 x 4 x 4 x 3 = 1200.
* Each combination also runs CNC approximately OFF for an NF A/B test.
*
* OUTPUTS (TEXT ONLY):
*   lna_pad_nc_sweep_summary.txt
*   lna_pad_nc_sweep_all.txt
*   lna_pad_nc_sweep_match_pass.txt
*   lna_pad_nc_sweep_full_pass.txt
*
* BAND SWEEP:
*   41 points, 2.3 to 2.5 GHz.
*   Index 20 is exactly 2.4 GHz.
*
* PASS DEFINITIONS:
*   MATCH_PASS: worst S11 in 2.3-2.5 GHz < -15 dB
*   GAIN_PASS : minimum S21 in 2.3-2.5 GHz > 15 dB
*   TB_STAB_PASS: mu > 1, mu' > 1, and K > 1 over target band
*   NC_PASS   : NF_OFF-NF_ON > 0.05 dB at 2.4 GHz and
*               NF_OFF_MAX-NF_ON_MAX > 0 dB
*   FULL_PASS : all four conditions above
*
* For FULL_PASS rows, a separate 10 MHz-30 GHz stability sweep is run.
* =============================================================================
...
echo GF180MCU_LNA_PAD_ACTIVE_NC_SWEEP > lna_pad_nc_sweep_summary.txt
echo TARGET_BAND_HZ 2.300e9 2.500e9 >> lna_pad_nc_sweep_summary.txt
echo CENTER_HZ 2.400e9 >> lna_pad_nc_sweep_summary.txt
echo FIXED_L_IN_H 1.000e-9 >> lna_pad_nc_sweep_summary.txt
echo FIXED_R_L_IN_OHM 1.257 >> lna_pad_nc_sweep_summary.txt
echo FIXED_L_BW_H 2.000e-9 >> lna_pad_nc_sweep_summary.txt
echo FIXED_R_BW_OHM 0.2 >> lna_pad_nc_sweep_summary.txt
echo FIXED_VBIAS_CG_V 2.25 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_M_CG 7 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_CBOOT_F 75e-15 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_CIN_F 22e-12 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_CNC_F 4.7e-12 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_RFF_OHM 22 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_ZIN_OHM 22.8215 J7.2873 >> lna_pad_nc_sweep_summary.txt
echo BASELINE_AVPAD 4.40251 J_MINUS3.65732 >> lna_pad_nc_sweep_summary.txt
echo REQUIRED_PAD_ADMITTANCE_CHANGE_S MINUS7.1676e-3 J_MINUS13.2040e-3 >> lna_pad_nc_sweep_summary.txt
echo LINEARIZED_EXTRA_CBOOT_FOR_G_ONLY_F 129.96e-15 >> lna_pad_nc_sweep_summary.txt
echo LINEARIZED_EXTRA_CBOOT_FOR_B_ONLY_F 257.34e-15 >> lna_pad_nc_sweep_summary.txt
echo NOTE_CBOOT_ALONE_CANNOT_MEET_BOTH_G_AND_B_SO_MCG_CIN_CNC_AND_RFF_ARE_SWEPT >> lna_pad_nc_sweep_summary.txt
echo SWEEP_M_CG 7 8 9 10 11 >> lna_pad_nc_sweep_summary.txt
echo SWEEP_CBOOT_F 75e-15 150e-15 225e-15 300e-15 375e-15 >> lna_pad_nc_sweep_summary.txt
echo SWEEP_CIN_F 4.7e-12 10e-12 15e-12 22e-12 >> lna_pad_nc_sweep_summary.txt
echo SWEEP_CNC_F 1e-12 2.2e-12 3.3e-12 4.7e-12 >> lna_pad_nc_sweep_summary.txt
echo SWEEP_RFF_OHM 22 47 68 >> lna_pad_nc_sweep_summary.txt

echo M_CG CBOOT_F CIN_F CNC_F RFF_OHM PDC_MW ID2_A GM2_S GMBS2_S GDS2_S RIN_CG_EST_OHM XM5_SAT_MARGIN_V S11_CENTER_DB S11_WORST_DB MATCH_PASS ZIN_RE_OHM ZIN_IM_OHM S21_CENTER_DB S21_MIN_DB GAIN_PASS S22_CENTER_DB S22_WORST_DB NF_ON_CENTER_DB NF_ON_MAX_DB NF_OFF_CENTER_DB NF_OFF_MAX_DB NF_IMPROVEMENT_CENTER_DB NF_IMPROVEMENT_WORST_DB NC_PASS MU_MIN MUP_MIN K_MIN TB_STAB_PASS AVPAD_RE AVPAD_IM AVPAD_MAG AVNC_RE AVNC_IM AVNC_MAG FULL_PASS > lna_pad_nc_sweep_all.txt

echo M_CG CBOOT_F CIN_F CNC_F RFF_OHM S11_CENTER_DB S11_WORST_DB ZIN_RE_OHM ZIN_IM_OHM S21_CENTER_DB S21_MIN_DB NF_ON_CENTER_DB NF_ON_MAX_DB NF_IMPROVEMENT_CENTER_DB NF_IMPROVEMENT_WORST_DB MU_MIN MUP_MIN K_MIN AVPAD_RE AVPAD_IM AVNC_RE AVNC_IM > lna_pad_nc_sweep_match_pass.txt

echo M_CG CBOOT_F CIN_F CNC_F RFF_OHM PDC_MW S11_CENTER_DB S11_WORST_DB ZIN_RE_OHM ZIN_IM_OHM S21_CENTER_DB S21_MIN_DB S22_CENTER_DB S22_WORST_DB NF_ON_CENTER_DB NF_ON_MAX_DB NF_OFF_CENTER_DB NF_OFF_MAX_DB NF_IMPROVEMENT_CENTER_DB NF_IMPROVEMENT_WORST_DB MU_MIN MUP_MIN K_MIN WB_MU_MIN WB_MUP_MIN WB_K_MIN WB_DELTA_MAX AVPAD_RE AVPAD_IM AVPAD_MAG AVNC_RE AVNC_IM AVNC_MAG > lna_pad_nc_sweep_full_pass.txt

* -----------------------------------------------------------------------------
* Nested coarse sweep
* -----------------------------------------------------------------------------

foreach mcg 7 8 9 10 11
  foreach cb 75f 150f 225f 300f 375f
    foreach ci 4.7p 10p 15p 22p
      foreach cn 1p 2.2p 3.3p 4.7p
        foreach rf 22 47 68

          * Apply active-noise-cancellation ON parameters.
          alterparam M_CG=$mcg
          alterparam CBOOT_VAL=$cb
          alterparam CIN_VAL=$ci
          alterparam CNC_VAL=$cn
          alterparam R_FF=$rf
          alterparam VBIAS_CG=2.25
          reset

          * ---------------------------------------------------------------
          * DC operating point for this combination
          * ---------------------------------------------------------------
          op

          let dc_pdc_mw=1e3*(abs(i(VDD))*v(vdcbias)+abs(i(VDDIO))*v(vddio))
          let dc_id2=abs(@m.xm2.m0[id])
          let dc_gm2=@m.xm2.m0[gm]
          let dc_gmb2=@m.xm2.m0[gmbs]
          let dc_gds2=@m.xm2.m0[gds]
          let dc_gds5=@m.xm5.m0[gds]
          let dc_rin_cg=1/(dc_gm2+dc_gmb2+dc_gds2+dc_gds5+1e-30)
          let dc_sat5=@m.xm5.m0[vds]-abs(@m.xm5.m0[vdsat])
          ...
          set o_gmb2=$&dc_gmb2
          set o_gds2=$&dc_gds2
          set o_rincg=$&dc_rin_cg
          set o_sat5=$&dc_sat5

          * ---------------------------------------------------------------
          * Active NC ON: target-band SP/noise sweep
          * ---------------------------------------------------------------
          sp lin 41 2.3Gig 2.5Gig 1
          ...
          let on_delta=s_1_1*s_2_2-s_1_2*s_2_1
          let on_dmag=mag(on_delta)
          let on_a11=mag(s_1_1)
          let on_a12=mag(s_1_2)
          let on_a21=mag(s_2_1)
          let on_a22=mag(s_2_2)
          let on_mu=(1-on_a11*on_a11)/(mag(s_2_2-on_delta*conj(s_1_1))+mag(s_1_2*s_2_1)+1e-30)
          let on_mup=(1-on_a22*on_a22)/(mag(s_1_1-on_delta*conj(s_2_2))+mag(s_1_2*s_2_1)+1e-30)
          let on_k=(1-on_a11*on_a11-on_a22*on_a22+on_dmag*on_dmag)/(2*on_a12*on_a21+1e-30)

          let on_s11c=on_s11db[20]
          let on_s11worst=maximum(on_s11db)
          let on_s21c=on_s21db[20]
          let on_s21min=minimum(on_s21db)
          let on_s22c=on_s22db[20]
          let on_s22worst=maximum(on_s22db)
          let on_nfc=on_nfdb[20]
          let on_nfmax=maximum(on_nfdb)
          let on_mumin=minimum(on_mu)
          let on_mupmin=minimum(on_mup)
          let on_kmin=minimum(on_k)

          let on_zinc=50*(1+s_1_1[20])/(1-s_1_1[20])
          let on_zinre=real(on_zinc)
          let on_zinim=imag(on_zinc)

          let on_avpad=v(vm2)[20]/v(rf_in_pad)[20]
          let on_avpadre=real(on_avpad)
          let on_avpadim=imag(on_avpad)
          let on_avpadmag=mag(on_avpad)

          let on_avnc=v(g_csc)[20]/v(s)[20]
          let on_avncre=real(on_avnc)
          let on_avncim=imag(on_avnc)
          let on_avncmag=mag(on_avnc)
          ...
          let on_tbstab_pass=0
          if on_mumin > 1
            if on_mupmin > 1
              if on_kmin > 1
                let on_tbstab_pass=1
              end
            end
          end

          * Save all ON scalars before reset.
          set o_s11c=$&on_s11c
          set o_s11worst=$&on_s11worst
          set o_matchpass=$&on_match_pass
          set o_zinre=$&on_zinre
          set o_zinim=$&on_zinim
          set o_s21c=$&on_s21c
          set o_s21min=$&on_s21min
          set o_gainpass=$&on_gain_pass
          set o_s22c=$&on_s22c
          set o_s22worst=$&on_s22worst
          set o_nfc=$&on_nfc
          set o_nfmax=$&on_nfmax
          set o_mumin=$&on_mumin
          set o_mupmin=$&on_mupmin
          set o_kmin=$&on_kmin
          set o_tbstabpass=$&on_tbstab_pass
          set o_avpadre=$&on_avpadre
          set o_avpadim=$&on_avpadim
          set o_avpadmag=$&on_avpadmag
          set o_avncre=$&on_avncre
          set o_avncim=$&on_avncim
          set o_avncmag=$&on_avncmag

          * ---------------------------------------------------------------
          * Active NC OFF A/B:
          * Keep XM7/XM9 biased and output loading retained.
          * Only open the sensing capacitor.
          * ---------------------------------------------------------------
          alterparam CNC_VAL=1e-18
          reset
          sp lin 41 2.3Gig 2.5Gig 1
          ...
          let off_nfmax=maximum(off_nfdb)
          let nfimp_c=off_nfc-$o_nfc
          let nfimp_worst=off_nfmax-$o_nfmax
          ...
          let full_pass=$o_matchpass*$o_gainpass*$o_tbstabpass*nc_pass

          set o_offnfc=$&off_nfc
          set o_offnfmax=$&off_nfmax
          set o_nfimp_c=$&nfimp_c
          set o_nfimp_worst=$&nfimp_worst
          set o_ncpass=$&nc_pass
          set o_fullpass=$&full_pass

          * Restore ON CNC value for next calculations / next iteration.
          alterparam CNC_VAL=$cn

          * ---------------------------------------------------------------
          * Write every combination
          * ---------------------------------------------------------------
          echo $mcg $cb $ci $cn $rf $o_pdc $o_id2 $o_gm2 $o_gmb2 $o_gds2 $o_rincg $o_sat5 $o_s11c $o_s11worst $o_matchpass $o_zinre $o_zinim $o_s21c $o_s21min $o_gainpass $o_s22c $o_s22worst $o_nfc $o_nfmax $o_offnfc $o_offnfmax $o_nfimp_c $o_nfimp_worst $o_ncpass $o_mumin $o_mupmin $o_kmin $o_tbstabpass $o_avpadre $o_avpadim $o_avpadmag $o_avncre $o_avncim $o_avncmag $o_fullpass >> lna_pad_nc_sweep_all.txt

          * Write all target-band S11 passing combinations.
          if $o_matchpass > 0
            echo $mcg $cb $ci $cn $rf $o_s11c $o_s11worst $o_zinre $o_zinim $o_s21c $o_s21min $o_nfc $o_nfmax $o_nfimp_c $o_nfimp_worst $o_mumin $o_mupmin $o_kmin $o_avpadre $o_avpadim $o_avncre $o_avncim >> lna_pad_nc_sweep_match_pass.txt
          end

          * ---------------------------------------------------------------
          * Wideband stability only for FULL_PASS combinations
          * ---------------------------------------------------------------
          if $o_fullpass > 0
            alterparam CNC_VAL=$cn
            reset
            sp dec 61 10Meg 30Gig 1

            let wb_delta=s_1_1*s_2_2-s_1_2*s_2_1
            let wb_dmag=mag(wb_delta)
            let wb_a11=mag(s_1_1)
            let wb_a12=mag(s_1_2)
            let wb_a21=mag(s_2_1)
            let wb_a22=mag(s_2_2)
            let wb_mu=(1-wb_a11*wb_a11)/(mag(s_2_2-wb_delta*conj(s_1_1))+mag(s_1_2*s_2_1)+1e-30)
            let wb_mup=(1-wb_a22*wb_a22)/(mag(s_1_1-wb_delta*conj(s_2_2))+mag(s_1_2*s_2_1)+1e-30)
            let wb_k=(1-wb_a11*wb_a11-wb_a22*wb_a22+wb_dmag*wb_dmag)/(2*wb_a12*wb_a21+1e-30)

            let wb_mumin=minimum(wb_mu)
            let wb_mupmin=minimum(wb_mup)
            let wb_kmin=minimum(wb_k)
            let wb_dmax=maximum(wb_dmag)

            set o_wbmumin=$&wb_mumin
            set o_wbmupmin=$&wb_mupmin
            set o_wbkmin=$&wb_kmin
            set o_wbdmax=$&wb_dmax

            echo $mcg $cb $ci $cn $rf $o_pdc $o_s11c $o_s11worst $o_zinre $o_zinim $o_s21c $o_s21min $o_s22c $o_s22worst $o_nfc $o_nfmax $o_offnfc $o_offnfmax $o_nfimp_c $o_nfimp_worst $o_mumin $o_mupmin $o_kmin $o_wbmumin $o_wbmupmin $o_wbkmin $o_wbdmax $o_avpadre $o_avpadim $o_avpadmag $o_avncre $o_avncim $o_avncmag >> lna_pad_nc_sweep_full_pass.txt
          end
...
* -----------------------------------------------------------------------------
* Restore the latest nominal values.
* -----------------------------------------------------------------------------

alterparam M_CG=7
alterparam CBOOT_VAL=75f
alterparam CIN_VAL=22p
alterparam CNC_VAL=4.7p
alterparam R_FF=22
alterparam VBIAS_CG=2.25
reset

echo SWEEP_COMPLETE >> lna_pad_nc_sweep_summary.txt
echo ALL_RESULTS lna_pad_nc_sweep_all.txt >> lna_pad_nc_sweep_summary.txt
echo MATCH_PASS_RESULTS lna_pad_nc_sweep_match_pass.txt >> lna_pad_nc_sweep_summary.txt
echo FULL_PASS_RESULTS lna_pad_nc_sweep_full_pass.txt >> lna_pad_nc_sweep_summary.txt
...
spice_ignore=true}
C {res.sym} 1020 -760 1 0 {name=R_FF
value=\{R_FF\}
footprint=1206
device=resistor
m=1}
C {capa.sym} 760 -370 0 0 {name=CBOOT
m=1
value=\{CBOOT_VAL\}
footprint=1206
device="ceramic capacitor"
spice_ignore=true}
C {lab_pin.sym} 760 -458.75 0 0 {name=p3 sig_type=std_logic lab=vm2}
C {lab_pin.sym} 760 -280 2 0 {name=p31 sig_type=std_logic lab=rf_in_pad}
